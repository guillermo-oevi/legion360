...
app.config['DEFAULT_GSHEET_ID'] = os.getenv('DEFAULT_GSHEET_ID', '1M7BLBqPM3rzrniaekB_EEoaRZ-NDTFp0phFkObRP5Qw')
...
def format_factnum(value):
    ...
    if not digits:
        return s
    if len(digits) <= 8:
        pv = '1'; num = digits.zfill(8)
    else:
        pv = digits[:-8] or '1'; num = digits[-8:].zfill(8)
    try:
        pv = str(int(pv))
    except Exception:
        pass
...
with app.app_context():
    ...
    with db.engine.begin() as conn:
        ...
        if 'personal' not in cols:
            conn.execute(text("ALTER TABLE compras ADD COLUMN personal INTEGER NOT NULL DEFAULT 0"))
...
def build_resumen_socio(ym: str):
    ...
    ventas_sub = db.session.query(Venta.socio_id.label('socio_id'), func.sum(Venta.pesos_sin_iva).label('ventas_sin_iva')).filter(Venta.ym == ym).group_by(Venta.socio_id).subquery()
    compras_sub = db.session.query(Compra.socio_id.label('socio_id'), func.sum(Compra.pesos_sin_iva).label('compras_sin_iva')).filter(Compra.ym == ym).group_by(Compra.socio_id).subquery()
    q = db.session.query(
    ...
    ).outerjoin(ventas_sub, ventas_sub.c.socio_id == Socio.id).outerjoin(compras_sub, compras_sub.c.socio_id == Socio.id)
    socios = [{'id': sid, 'nombre': nombre, 'tipo': tipo, 'gn': float(gn or 0.0)} for sid, nombre, tipo, gn in q.all()]
    filas = []
    for s in socios:
        gn = s['gn']
        tipo = s['tipo']
...
def index():
    ...
    c = db.session.query(func.coalesce(func.sum(Compra.pesos_sin_iva), 0.0), func.coalesce(func.sum(Compra.iva_21 + Compra.iva_105), 0.0)).filter(Compra.ym == ym).first()
    compras_sin_iva, iva_compra_total = float(c[0]), float(c[1])
    iva_personal_total = float(db.session.query(func.coalesce(func.sum(Compra.iva_21 + Compra.iva_105), 0.0)).filter((Compra.ym == ym) & (Compra.personal == True)).scalar() or 0.0)
    p_norm = get_param('iva_deducible_normal_pct', 1.0)
    p_pers_def = get_param('iva_deducible_personal_default_pct', 0.5)
    rows = db.session.query(Compra.personal, Compra.iva_deducible_pct, Compra.iva_21, Compra.iva_105).filter(Compra.ym == ym).all()
    ...
    ventas_sub = db.session.query(Venta.socio_id.label('socio_id'), func.sum(Venta.pesos_sin_iva).label('ventas_sin_iva')).filter(Venta.ym == ym).group_by(Venta.socio_id).subquery()
    compras_sub = db.session.query(Compra.socio_id.label('socio_id'), func.sum(Compra.pesos_sin_iva).label('compras_sin_iva')).filter(Compra.ym == ym).group_by(Compra.socio_id).subquery()
    q = db.session.query(Socio.nombre, func.coalesce(ventas_sub.c.ventas_sin_iva, 0.0), func.coalesce(compras_sub.c.compras_sin_iva, 0.0)).outerjoin(ventas_sub, ventas_sub.c.socio_id == Socio.id).outerjoin(compras_sub, compras_sub.c.socio_id == Socio.id)
    per_socio = [{
        'nombre': nombre,
    ...
    return render_template('index.html',
        ...
        compras_tot={'monto_total': compras_sin_iva + iva_compra_total, 'iva': iva_compra_total, 'iva_deducible': iva_compra_creditable},
        ...
        iva_personal_credito_socios=max(iva_personal_total - iva_personal_credito_empresa, 0.0),
...
def dashboard_export():
    ...
    ).filter(Compra.ym == ym).all()

    iva_compra_creditable, iva_personal_total, iva_personal_credito_empresa = 0.0, 0.0, 0.0
    for personal, pct, i21, i105 in rows:
        base = float((i21 or 0.0) + (i105 or 0.0))
        eff = float(pct if pct is not None else (p_pers_def if personal else p_norm))
        eff = min(max(eff, 0.0), 1.0)
    ...
    resumen = [{
        ...
        'Compras_ADEUDADO': int(db.session.query(func.count(Compra.id)).filter((Compra.ym == ym) & (Compra.estado == 'ADEUDADO')).scalar() or 0),
        'Ventas_ADEUDADO': int(db.session.query(func.count(Venta.id)).filter((Venta.ym == ym) & (Venta.estado == 'ADEUDADO')).scalar() or 0)
    }]
...
def resumen_arca():
    ...
    return render_template('resumen_arca.html', filas=filas, ym=ym or '', ym_list=all_dates, tipo=tipo or '', incluirN=int(incluirN))
...
def resumen_arca_export():
    ...
    if fmt == 'xlsx':
        ...
        return Response(sio.getvalue(), mimetype='text/csv', headers={'Content-Disposition': f"attachment; filename=resumen_arca_{ym or 'all'}{('_'+tipo) if tipo else ''}{'_inclN' if incluirN else ''}.csv"})
...
def totales_arca():
    ...
    return render_template('totales_arca.html', filas=agg, ym=ym or '', ym_list=ym_list, tipo=tipo or '', incluirN=int(incluirN))
...
def totales_arca_export():
    ...
    if fmt == 'xlsx':
        ...
        sio = io.StringIO()
        if agg:
            writer = csv.DictWriter(sio, fieldnames=agg[0].keys()); writer.writeheader(); writer.writerows(agg)
        return Response(sio.getvalue(), mimetype='text/csv', headers={'Content-Disposition': f"attachment; filename=totales_arca_{ym or 'all'}{('_'+tipo) if tipo else ''}{'_inclN' if incluirN else ''}.csv"})
...
def resumen_socio_view():
    ...
    if not ym:
        flash('No hay datos cargados aún. Importá el Excel/Sheet para ver el resumen.', 'warning')
...
def do_import_excel_from_path(path: str):
    ...
    try:
        ...
        pass
    deleted_c = db.session.query(Compra).filter(Compra.ym.in_(list(yms_c))).delete(synchronize_session=False) if yms_c else 0
    deleted_v = db.session.query(Venta).filter(Venta.ym.in_(list(yms_v))).delete(synchronize_session=False) if yms_v else 0
    ...
    for _, r in df_c.iterrows():
        try:
            ...
            if socio_oblig and not socio_id:
                ...
                continue
            personal = _to_bool_si_no(r.get('personal')) if ('personal' in df_c.columns) else False
            p_norm = get_param('iva_deducible_normal_pct', 1.0)
            p_pers_def = get_param('iva_deducible_personal_default_pct', 0.5)
    ...
    if rechazos:
        ...
        rej_file = fpath
    return {'deleted_c': deleted_c, 'deleted_v': deleted_v, 'rechazos': len(rechazos), 'rechazos_path': rej_file}
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if res['deleted_c'] or res['deleted_v']:
                flash(f"Limpieza previa: Compras {res['deleted_c']}, Ventas {res['deleted_v']}", 'info')
            if res['rechazos']:
                flash(f"Importación completa con {res['rechazos']} filas rechazadas.", 'warning')
                if res['rechazos_path']:
                    flash(f"Descargá el detalle: /uploads/{os.path.basename(res['rechazos_path'])}", 'info')
            else:
        ...
        return redirect(url_for('import_xls'))
    return render_template('import_xls.html', default_gsheet_id=app.config['DEFAULT_GSHEET_ID'])
...
def import_gsheet():
    ...
    try:
        ...
        if res['deleted_c'] or res['deleted_v']:
            flash(f"Limpieza previa: Compras {res['deleted_c']}, Ventas {res['deleted_v']}", 'info')
        if res['rechazos']:
            flash(f"Importación (Google Sheets) completa con {res['rechazos']} filas rechazadas.", 'warning')
            if res['rechazos_path']:
                flash(f"Descargá el detalle: /uploads/{os.path.basename(res['rechazos_path'])}", 'info')
        else:
